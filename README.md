...
- `use_map_saver` is only turned on in `mapping` mode, so the map is only
...
- rf2o used to only sample the lidar->root transform once, on its first
  received scan, and reuse that cached transform for its lifetime, which
  assumed a rigidly-fixed sensor mount the head-mounted lidar isn't. That's fixed in Thornbots/rf2o_laser_odometry (the fork
...
confirm this didn't cost genuine noise-responsiveness. It still passes
...
0.395-0.443m before). The wobble turned out not to be obstacle-related at
...
`odom1_config` (from `/scan_odom`) fuses position only, with yaw deliberately
...
irrelevant either way, since this chassis is holonomic and never rotates its
...
0.42–0.99m, never once near 0.20m. Instructively, the variants that
...
knob helped or hurt on `drift_correction`(`_obstacle`) specifically, since
the signal is currently smaller than the noise this host can inject. The
...
preceded the worst result, 0.9972m), so this run-to-run spread looks
...
argument that it *can't*, since rf2o measurably supplies a real,
~97%-accurate absolute-position correction, but two closed tuning directions
...
*worse* (0.89m), so it was reverted; raising it to 1.0 instead (fewer,
...
odom alignment) gave no improvement over the search-space+interval change
alone (0.59m vs 0.50m, within this scenario's own run-to-run spread), so
that was reverted too, keeping only the two changes above.
...
threshold combination, the same likely floor as `amcl`: a single-shot,
...
through, `git show 8a719ab:config/ekf.yaml` confirms it matches). That
...
four scenarios run before `noise_correction` in suite order), so it's worth flagging
