...
(--backend slam/amcl/ekf, default slam -- not mapping, see BACKENDS below
...
-------------------------------------------------------
sentry_localization/test/ already has ament_copyright/ament_flake8/
ament_pep257 pytest-style tests, run via `colcon test`. Those are fast,
single-process, static-analysis-style checks with no external state. This
suite is the opposite on every axis that matters for choosing a test runner:
...
dexec.sh's env sourcing already provides), from the host:
    ...
    isaac_ros_common/scripts/dexec.sh -- \\
        python3 /workspaces/isaac_ros-dev/src/sentry_localization/test/slam_integration/run_localization_drift_tests.py
...
# on both sides regardless of the exact mount height.
